...
var questions = [
	...
	},
	{
		...
		"options": [
			"Uma injeção de código SQL maliciosa",
			"Um tipo de banco de dados",
			"Um comando para backup"
		],
		"correct": 0,
	...
	},
	{
		...
		"options": [
			...
			"São formas de declarar variáveis com diferentes escopos",
			"Apenas var funciona em JavaScript"
		],
		"correct": 1,
	...
	},
	{
		...
		"options": [
			...
			"Organizar dados em tabelas relacionadas",
			"Criar animações"
		],
		"correct": 1,
...
func start_intro_dialogue():
	dialog_active = true
	await show_dialogue("Billet", "Você sente o peso, não sente? O peso de tudo que ainda precisa fazer. O peso de não poder falhar.", "res://sprites/characters/Boleto/billet_face.jpg")
	await show_dialogue("Yume", "Eu carrego isso todo dia. Não é de hoje.", "res://dialog/assets/yumeFace.png")
	await show_dialogue("Billet", "Você acha que aguentar o peso é suficiente? Não é. Saber carregar... isso é o que separa os que formam dos que desistem.", "res://sprites/characters/Boleto/billet_face.jpg")
	await show_dialogue("Yume", "E você acha que eu vou desistir agora?", "res://dialog/assets/yumeFace.png")
	await show_dialogue("Billet", "Não acho. Eu vou garantir. Cada erro seu... o céu vai cair. Esse é o peso do amanhã que você não conseguiu suportar.", "res://sprites/characters/Boleto/billet_face.jpg")
	await show_dialogue("Yume", "Então vamos. Me mostre o que eu não sei.", "res://dialog/assets/yumeFace.png")
	
	dialog_active = false
	ask_next_question()

@warning_ignore("shadowed_variable_base_class")
func show_dialogue(name, text, face_path):
	if is_game_over:
		return
...
func ask_next_question():
	...
	dialog_active = true
	await show_dialogue("Billet", q.text, "res://sprites/characters/Boleto/billet_face.jpg")
	dialog_active = false
...
func check_answer(selected_index):
	...
	if is_correct:
		hits += 1
		dialog_active = true
		await show_dialogue("Billet", q.right_comment, "res://sprites/characters/Boleto/billet_face.jpg")
		dialog_active = false
		...
		errors += 1
		dialog_active = true
		await show_dialogue("Billet", q.wrong_comment, "res://sprites/characters/Boleto/billet_face.jpg")
		dialog_active = false
		
		# SPAWN DOS PROJÉTEIS DIRETO NA CENA
		spawn_projectiles()
...
func spawn_projectiles():
	print("=== CORTINA IMPENETRÁVEL: SEM ESPAÇO PARA DESVIAR ===")
	
	if is_game_over:
		return
	
	if projectile_scene == null:
		print("ERRO: projectile_scene é NULL")
		return
	
	var main_scene = get_tree().current_scene
	if main_scene == null:
		print("ERRO: main_scene é NULL")
		return
	
	var screen_width = get_viewport().size.x
	
	# CRIA UMA PAREDE DE PROJÉTEIS (cobre TODA a largura)
	var espacamento = 25  # Espaço entre cada projétil (pequeno = sem brecha)
	var quantidade_por_linha = int(screen_width / espacamento) + 1
	
	# 5 LINHAS DE PROJÉTEIS (uma atrás da outra)
	for linha in range(5):
		for i in range(quantidade_por_linha):
			var projectile = projectile_scene.instantiate()
			main_scene.add_child(projectile)
			
			# Posição X: cobre toda a tela sem espaços
			var pos_x = i * espacamento
			# Posição Y: linhas em alturas diferentes
			var pos_y = -50 + (linha * 80)
			
			projectile.position = Vector2(pos_x, pos_y)
		
		# Delay mínimo entre linhas
		await get_tree().create_timer(0.05).timeout
	
	# RAJADA EXTRA: mais 3 linhas mais baixas (pra pegar quem pular)
	for linha in range(3):
		for i in range(quantidade_por_linha):
			var projectile = projectile_scene.instantiate()
			main_scene.add_child(projectile)
			var pos_x = i * espacamento
			var pos_y = 200 + (linha * 60)  # Altura do meio da tela
			projectile.position = Vector2(pos_x, pos_y)
...
func victory():
	...
	dialog_active = true
	set_buttons_enabled(false)
	await show_dialogue("Billet", "Impossível... você carregou o peso... e não quebrou...", "res://sprites/characters/Boleto/billet_face.jpg")
	await show_dialogue("Yume", "O peso do amanhã não é uma ameaça. É uma promessa. E eu vou cumprir a minha.", "res://dialog/assets/yumeFace.png")
...
func game_over():
	...
	dialog_active = true
	set_buttons_enabled(false)
	await show_dialogue("Billet", "Eu avisei. O peso era grande demais pra você.", "res://sprites/characters/Boleto/billet_face.jpg")
	await show_dialogue("Yume", "Eu... não estudei... o suficiente...", "res://dialog/assets/yumeFace.png")
	await show_dialogue("Billet", "Não adianta chorar agora. O amanhã já chegou... e você não estava pronta.", "res://sprites/characters/Boleto/billet_face.jpg")
